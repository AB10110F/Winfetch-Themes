...
function info_cpu {
    ...
    return @{
        icon    = "󰻠  "
        title   = "CPU"
        content = "$($cpuname.TrimStart("Intel(R) Core(TM)")) @ $($cpu.GetValue("~MHz") / 1000)GHz" # [math]::Round($cpu.GetValue("~MHz") / 1000, 1) is 2-5ms slower
        # content = "$($cpuname.TrimStart("Intel(R) Core(TM)"))" # [math]::Round($cpu.GetValue("~MHz") / 1000, 1) is 2-5ms slower
    }
}
...
function info_memory {
    ...
    return @{
        ...
        content =  "$($used.ToString("#.##")) GB | $($total.ToString("#.##")) GB"
    }
}
...
# move cursor to top of image and to its right (here)
...
foreach ($item in $config) {
    ...
    foreach ($line in $info) {
        if ($line["icon"]) { # add icons by bro
          $output = " │  $e[1;37m$($line["icon"])"
        }
        else{
          $output = ""
        }
